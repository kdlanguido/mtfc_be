...
export class PromotionsService {
  ...
  async findAll() {
    try {
      const Promotions = await this.promotionModel.find().sort({ _id: -1 }).exec()
      if (!Promotions) {
        return {
  ...
  async findByTarget(target: string) {
    try {
      const Promotions = await this.promotionModel.find({ target }).sort({ _id: -1 }).exec()
      if (!Promotions) {
        return {
          message: "No promotions found",
          status: 404
  ...
  async findByTitle(title: string) {
    try {
      const Promotions = await this.promotionModel.find({ title: { $regex: title, $options: 'i' } }).sort({ _id: -1 }).exec()
      if (!Promotions) {
        return {
          message: "No promotions found",
          status: 404
  ...
  async update(id: string, updatePromotionDto: UpdatePromotionDto) {
    try {
      const updatedPromotion = this.promotionModel.findByIdAndUpdate(id,
        updatePromotionDto,
        {
          new: true,
          runValidators: true,
